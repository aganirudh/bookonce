...
import type { ImpactAnalysis } from './disruptionTypes';
...
export function JourneyResiliencePanel({ state, explicitDependencies = [], onApply }: {
  ...
  const simulate = async () => {
    ...
    setImpact(result.analysis); setRecovery(nextRecovery); setSelected(nextRecovery.recommendedCandidate);
  ...
  const apply = () => {
    ...
    if (!result.success) { setStatus(['stale-plan', 'stale-version', 'invalid-version'].includes(result.error) ? 'Recovery plan is outdated. Please recalculate.' : 'Recovery plan could not be applied.'); return; }
  ...
  return <Card data-testid="resilience-panel">
    ...
    <CardContent className="space-y-4">
      {!impact && <p>Disruption monitoring not connected.</p>}
      ...
      {impact && <section aria-labelledby="impact-heading" className="space-y-2">
        <div className="flex items-center gap-2"><AlertTriangle className="h-5 w-5" /><h3 id="impact-heading" className="font-semibold">Simulation impact</h3><DataProvenanceBadge provenance="simulation" /></div>
      ...
      {impact && <Button type="button" variant="outline" onClick={dismiss}>Dismiss simulation</Button>}
